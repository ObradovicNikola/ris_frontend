<template>
  <div>
    <v-alert v-if="error" border="left" prominent type="error" class="mb-5">
      {{ error }}
    </v-alert>
    <h1 class="text-h4">
    ...
    <h2 class="subtitle-1 mt-1">email: {{ user.email }}</h2>
        ...
        <v-card color="grey lighten-4" min-width="350px" flat>
          ...
          <v-card-text class="black--text">
            <div class="d-flex flex-column">
              ...
              <span v-if="selectedEvent.ocena && selectedEvent.ocena >= 0">
                ocena: {{ selectedEvent.ocena }}
              </span>
            </div>
          </v-card-text>
          <v-card-actions>
...
const asyncData = async function ({ $axios, params }) {
  try {
    const user = await $axios.$get(`api/admin/getuser/${params.id}`)
    const activities = await $axios.$get(
      `api/admin/getuser/${params.id}/aktivnosti`
    )
